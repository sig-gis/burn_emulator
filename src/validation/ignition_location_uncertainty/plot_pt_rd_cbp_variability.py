...
PS_SCENARIO = "s2"
PS_OUTPUT_DIR = f"{PS_DATA_DIR}/{PS_SCENARIO}/{PS_OUTPUT_DIR_NAME}"
...
for proj_id in project_areas_gdf["proj_id"][:3]:
    ...
    # use first iteration for representative ignitions
    ignitions_df = pd.read_csv(f"{PS_OUTPUT_DIR}/iteration_0/{proj_id}_baseline/{proj_id}_ignitions_locations.csv")
    ...
    plt.suptitle(f"PT-CBP Relative Difference for Scenario {PS_SCENARIO}, Project Area {proj_id}")
    plt.savefig(f"/tmp/pt_{PS_SCENARIO}-{proj_id}_rd_CBP_il_uncertainty.png")
